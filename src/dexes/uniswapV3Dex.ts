...
class UniswapV3Dex extends BaseDex {
    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            logger.error(`Error initializing AFLAB contract: ${error}`, this.constructor.name);
            throw error;
        }
        ...
        try {
            ...
            logger.error(`Error initializing LendingPoolAP contract: ${error}`, this.constructor.name);
            throw error;
        }
        ...
        try {
            ...
            logger.error(`Error initializing subgraph: ${error}`, this.constructor.name);
            throw error;
        }
        ...
        for (const pool of this.pools) {
            ...
            try {
                ...
                logger.error(`Error initializing pool contract: ${error}`, this.constructor.name);
                throw error;
            }
            logger.info(`Initialized pool for contract: ${pool.id}`, this.constructor.name);
    ...
    async processSwap(swap: UniswapV3Swap, lastPoolSqrtPriceX96: BigNumber) {
        ...
        if (lastPoolSqrtPriceX96 <= BigNumber.from(0)) {
            logger.warn(`Invalid lastPoolSqrtPriceX96: ${lastPoolSqrtPriceX96}`, this.constructor.name);
            return;
        }
        ...
        if (isPriceImpactSignificant(opportunity.originalSwapPriceImpact)) {
            await this.handleSignificantPriceImpact(opportunity, tokenA, tokenC, swapName, contract);
        }
    }
    ...
    private async handleSignificantPriceImpact(
        ...
        if (candidateTokenBs.length === 0) {
            logger.info(`No candidates for token B found for opportunity.`, this.constructor.name);
            return;
        }

        logger.info(`Found ${candidateTokenBs.length} candidate token Bs`, this.constructor.name);

        let tokenBData;
        try {
            tokenBData = await this.pickTokenB(tokenA, tokenC, candidateTokenBs, opportunity.tokenAIn, contract);
        } catch (error) {
            logger.debug(`Unable to pick token B: ${error}`, this.constructor.name);
            logger.info(`No profitable arbitrage opportunities found for swap: ${swapName}`, this.constructor.name);
            return;
        }
        ...
        if (expectedProfit.expectedProfit.lte(0)) {
            logger.error(`Negative expected profit: ${expectedProfit.expectedProfit}`, this.constructor.name);
            return;
        }
