...
import { Token, Pool, ExpectedProfitData } from "../../../src/types.js";
...
it("should throw an error if contract not found", () => {
  expect(() => dex.getContract("nonExistentPool")).toThrowError(
    "Contract not found: nonExistentPool"
  );
});
...
it("should calculate expected profit for a three-step arbitrage if profitable", async () => {
  ...
  mockPoolContract.getLastPoolSqrtPriceX96.mockReturnValue(
