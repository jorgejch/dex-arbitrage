...
describe('UniswapV3Dex Class', () => {
    ...
    const subgraphMock = {
        initialize: () => Promise.resolve(), getPools: () => Promise.resolve([])
    ...
    it('should initialize correctly', async () => {
        const dex = new UniswapV3Dex(
            alchemyMock, walletMock, subgraphMock, aflabContractMock, lendingPoolAPContractMock, networkIdMock);

        const spyAflabInitialize = vi.spyOn(aflabContractMock, 'initialize')
                                     .mockImplementation(() => Promise.resolve());
        const spyLendingPoolInitialize = vi.spyOn(lendingPoolAPContractMock, 'initialize')
                                           .mockImplementation(() => Promise.resolve());
    ...
    it('should process swap events', async () => {
        ...
        const swapMock = {
            getContractAddress: () => '', setTokens: () => {}, amount0: 100,  // Mock some valid amount
            amount1: 200,  // Mock some valid amount
            calculatePriceImpact: () => 100, handleSignificantPriceImpact: () => Promise.resolve(),
        };

        const tokenMock = {symbol: 'TOKEN', decimals: 18} as Token;
        const spyGetContract = vi.spyOn(dex, 'getContract')
                                 .mockImplementation(() => ({
        ...
        const spyHandleSignificantPriceImpact = vi.spyOn(dex, 'handleSignificantPriceImpact' as any)
                                                  .mockImplementation(() => Promise.resolve());
